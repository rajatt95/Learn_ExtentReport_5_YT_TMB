...
public class ExtentReports_5_AttachScreenshot_AsBase64_SimpleWay {

	private static final String SRC_TEST_RESOURCES_CONFIG_XML = ".\\src\\test\\resources\\config.xml";
	private static final String REPORTS_SPARK_ATTACH_LOGO_USING_XML_HTML = "reports/Spark_AttachScreenshot_AsBase64_SimpleWay.html";
	...
	public void extent_attachScreenshotTest() {
		...
		try {
			test1.pass("Value Entered: ",
					MediaEntityBuilder.createScreenCaptureFromBase64String(Utilities.getBase64(driver)).build());
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		/*
		 * test1.assignAuthor("Rajat").assignCategory("Smoke").assignCategory(
		 * "Regression").assignDevice("Chrome")
